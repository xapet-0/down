...
class GeminiBot:
    ...
    def send_batch(self, page, blocks):
        ...
        if self._last_sent == msg:
            self.logger.warn("Duplicate payload detected. Skipping send to avoid loops.")
            return False
        ...
        try:
            ...
            if not textarea.inner_text().startswith("SYSTEM:") and pyperclip:
                pyperclip.copy(msg)
...
class EagleProcessor:
    ...
    def process_file(self, file_path, bot, page):
        ...
        while i < len(total_blocks):
            ...
            send_success = bot.send_batch(page, chunk)
            if not send_success:
                self.logger.warn("Send failed. Retrying once after short delay...")
                time.sleep(4)
                send_success = bot.send_batch(page, chunk)
                if not send_success:
                    if not self._manual_override("Send failed twice. Manual continue?"):
                        return
                    continue

            finished = bot.wait_for_completion(page)
            if not finished:
                self.logger.warn("Gemini did not finish in time. Retrying wait...")
                finished = bot.wait_for_completion(page)
                if not finished and not self._manual_override(
                    "Gemini timeout twice. Manual copy?"
                ):
                    return
            ...
            if not response_text.strip():
                self.logger.warn("Auto-copy failed. Requesting manual copy...")
                response_text = self._manual_copy()
            ...
            if len(srt_matches) >= required:
                ...
                self.logger.warn(
                    f"Mismatch ({len(srt_matches)}/{len(chunk)}). Retrying once..."
                )
                response_text = self._manual_copy()
                srt_matches = TranscriptParser.extract_srt_blocks(response_text)
                if len(srt_matches) >= required:
                    ...
                    if not self._manual_override("Manual paste mismatch. Retry batch?"):
                        return
                    time.sleep(2)
    ...
    def _manual_copy(self):
        prompt = "Copy translation then press Enter..."
        if not pyperclip:
            prompt = "Copy translation then press Enter (pyperclip missing)..."
        input(prompt)
        return read_clipboard_text()

    def _manual_override(self, prompt):
        choice = input(f"{prompt} (Y to continue): ").strip().upper()
        return choice == "Y"
