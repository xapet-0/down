...
# =======================================================
# 🦅 EAGLE V2: INDUSTRIAL-GRADE KEEPER
# =======================================================
...
class ManualDialog(tk.Toplevel):
    def __init__(self, parent, prompt):
        ...
        ttk.Label(self, text=prompt, wraplength=560).pack(pady=10)
        self.text_area = scrolledtext.ScrolledText(self, height=8, wrap="word")
        self.text_area.pack(fill="both", expand=True, padx=10)
        ttk.Button(self, text="Confirm", command=self._on_confirm).pack(pady=10)

    def _on_confirm(self):
        self.result_text = self.text_area.get("1.0", "end").strip()
        self.destroy()
...
class EagleGUI:
    def __init__(self, config):
        self.config = config
        self.root = tk.Tk()
        self.root.title("🦅 Eagle V2 - Industrial Subtitle Keeper")
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="🦅 Eagle V2", font=("Segoe UI", 16, "bold")).pack(
            side="left"
        )
        ttk.Label(
    ...
    def _manual_dialog(self, prompt):
        if self.manual_text.get("1.0", "end").strip():
            text = self.manual_text.get("1.0", "end").strip()
            self.manual_text.delete("1.0", "end")
            return text
        dialog = ManualDialog(self.root, prompt)
...
def run_cli(config):
    ...
    def manual_provider(prompt):
        print(prompt)
        return input("Paste translation: ")
...
def main():
    parser = argparse.ArgumentParser(description="Eagle V2 - Subtitle Translator")
